...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}:root{--admin-sidebar-w:240px;--admin-topbar-h:60px;--admin-bg:#0d0f14;--admin-sidebar-bg:#13161e;--admin-card:#1a1d27;--admin-border:rgba(255,255,255,.07);--admin-text:#e4e6f0;--admin-text-muted:#7b7f9e;--admin-primary:#6c63ff;--admin-primary-hover:#574fd6;--admin-success:#22c55e;--admin-danger:#ef4444;--admin-warning:#f59e0b;--admin-info:#3b82f6;--admin-radius:10px}
    ...
    .form-group{margin-bottom:20px}.form-label{display:block;font-size:13px;font-weight:600;color:var(--admin-text);margin-bottom:7px}.form-label span{color:var(--admin-danger);margin-left:2px}.form-hint{font-size:11.5px;color:var(--admin-text-muted);margin-top:5px}.form-control{width:100%;padding:10px 14px;background:rgba(255,255,255,.05);border:1px solid var(--admin-border);border-radius:8px;color:var(--admin-text);font-family:'Inter',sans-serif;font-size:13.5px;outline:none}.form-control:focus{border-color:var(--admin-primary);box-shadow:0 0 0 3px rgba(108,99,255,.2)}.form-control option{background:#171a22;color:var(--admin-text)}.form-control.is-invalid{border-color:var(--admin-danger)}textarea.form-control{resize:vertical;min-height:100px}.invalid-feedback{color:var(--admin-danger);font-size:12px;margin-top:5px;display:block}.btn-admin{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:8px;font-size:13.5px;font-weight:600;cursor:pointer;border:none;text-decoration:none;transition:.15s;line-height:1}.btn-admin-primary{background:var(--admin-primary);color:#fff}.btn-admin-danger{background:var(--admin-danger);color:#fff}.btn-admin-success{background:var(--admin-success);color:#fff}.btn-admin-ghost{background:rgba(255,255,255,.06);color:var(--admin-text);border:1px solid var(--admin-border)}.btn-sm{padding:6px 10px;font-size:11.5px}.admin-alert{display:flex;align-items:flex-start;gap:12px;padding:14px 18px;border-radius:9px;margin-bottom:20px;font-size:13.5px;font-weight:500}.admin-alert-success{background:rgba(34,197,94,.12);border:1px solid rgba(34,197,94,.25);color:#4ade80}.admin-alert-error{background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.25);color:#f87171}.admin-alert-warning{background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.25);color:#fbbf24}.pagination-wrap{display:flex;justify-content:flex-end;margin-top:20px;overflow-x:auto}.pagination-wrap .pagination{display:flex;gap:4px}.pagination-wrap .page-link{display:flex;align-items:center;justify-content:center;min-width:34px;height:34px;padding:0 8px;border-radius:7px;font-size:13px;font-weight:600;text-decoration:none;color:var(--admin-text-muted);background:rgba(255,255,255,.05);border:1px solid var(--admin-border)}.color-swatch{display:inline-block;width:18px;height:18px;border-radius:4px;border:1px solid rgba(255,255,255,.15);vertical-align:middle;margin-right:6px}.avatar-preview{width:64px;height:64px;border-radius:50%;object-fit:cover;border:2px solid var(--admin-border)}.avatar-placeholder{width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#6c63ff,#ff2a6d);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:#fff}.admin-stats-row,.admin-stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;margin-bottom:24px}.admin-stat-card{background:var(--admin-card);border:1px solid var(--admin-border);border-radius:var(--admin-radius);padding:16px 18px;min-width:0;overflow:hidden}.admin-stat-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.8px;color:var(--admin-text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.admin-stat-value{font-size:23px;font-weight:800;color:var(--admin-text);margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:-.3px}.admin-stat-value.primary{color:var(--admin-primary)}.modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.65);backdrop-filter:blur(4px);z-index:999;align-items:center;justify-content:center;padding:16px}.modal-overlay.show{display:flex}.modal-box{background:var(--admin-card);border:1px solid var(--admin-border);border-radius:14px;padding:28px 30px;max-width:400px;width:90%;text-align:center;max-height:90vh;overflow:auto}.modal-icon{font-size:42px;margin-bottom:12px}.modal-title{font-size:18px;font-weight:700;margin-bottom:8px}.modal-desc{font-size:13.5px;color:var(--admin-text-muted);margin-bottom:22px}.modal-actions{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}
    ...
    @media(max-width:860px){.admin-sidebar{transform:translateX(-100%);width:min(280px,86vw)}.admin-sidebar.open{transform:translateX(0)}.admin-topbar{left:0;padding:0 12px}.admin-main{margin-left:0;padding:20px 16px 36px}.mobile-menu-btn{display:inline-grid;place-items:center}.topbar-actions>:not(form){display:none}.admin-card{padding:18px}.ph h1,.admin-page-title{font-size:20px}}
    @media(max-width:520px){.admin-main{padding:16px 10px 30px}.admin-card{padding:14px;border-radius:9px}.admin-topbar{height:56px}.topbar-breadcrumb{font-size:12px}.topbar-btn{padding:7px 9px;font-size:11px}.admin-stat-value{font-size:22px}.admin-stats-row,.admin-stats-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.admin-stat-card{padding:12px}.modal-box{padding:20px 16px}}
  </style>
  @stack('styles')
</head>
...
<header class="admin-topbar"><div style="display:flex;align-items:center;gap:10px;min-width:0"><button type="button" class="mobile-menu-btn" id="mobile-menu-btn" aria-label="Mở menu">☰</button><div class="topbar-breadcrumb">Admin / <span>@yield('breadcrumb','Dashboard')</span></div></div><div class="topbar-actions">@yield('topbar-actions')<form method="POST" action="{{ route('logout') }}" style="margin:0">@csrf<button type="submit" class="topbar-btn topbar-btn-danger">✕ Đăng xuất</button></form></div></header>
<main class="admin-main">@if(session('success'))<div class="admin-alert admin-alert-success" id="flash-msg"><span>✅</span><span>{{ session('success') }}</span></div>@endif @if(session('error'))<div class="admin-alert admin-alert-error" id="flash-msg"><span>❌</span><span>{{ session('error') }}</span></div>@endif @if(session('warning'))<div class="admin-alert admin-alert-warning" id="flash-msg"><span>⚠️</span><span>{{ session('warning') }}</span></div>@endif @yield('content')</main>
<div class="modal-overlay" id="delete-modal"><div class="modal-box"><div class="modal-icon">🗑️</div><div class="modal-title">Xác nhận xóa</div><div class="modal-desc" id="delete-modal-desc">Bạn có chắc chắn muốn xóa mục này không?</div><div class="modal-actions"><button type="button" class="btn-admin btn-admin-ghost" onclick="closeDeleteModal()">Hủy bỏ</button><form id="delete-form" method="POST" style="display:inline">@csrf @method('DELETE')<button type="submit" class="btn-admin btn-admin-danger">Xóa ngay</button></form></div></div></div>
<script>
  const flash=document.getElementById('flash-msg');if(flash)setTimeout(()=>flash.style.opacity='0',4000);
  function confirmDelete(action,itemName){const modal=document.getElementById('delete-modal');document.getElementById('delete-form').action=action;document.getElementById('delete-modal-desc').textContent=itemName?`Bạn có chắc muốn xóa "${itemName}"?`:'Bạn có chắc chắn muốn xóa mục này không?';modal.classList.add('show')}
