...
<main class="page-container"><div class="container" style="padding-top:32px;padding-bottom:56px;">
  <div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap;margin-bottom:18px"><div><h1 style="margin:0 0 6px">📚 Tủ Truyện</h1><p style="margin:0;color:var(--text-sub)">Những bộ m đang theo dõi, lưu bằng tài khoản thật chứ không còn nằm trong localStorage của một chiếc trình duyệt cô đơn.</p></div><span style="font-size:13px;color:var(--text-sub)">{{ $libraries->total() }} bộ truyện</span></div>
  @include('user._nav')

  @if(!empty($stats))
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:22px">@foreach([['Theo dõi',$stats['library_count'] ?? $libraries->total()],['Đã đọc',$stats['history_count'] ?? 0],['Chương đã đọc',$stats['chapters_read'] ?? 0]] as [$label,$value])<div style="padding:14px;background:var(--bg-surface-1);border:1px solid var(--border-color);border-radius:12px"><strong style="font-size:22px;display:block">{{ number_format($value) }}</strong><span style="font-size:11px;color:var(--text-sub)">{{ $label }}</span></div>@endforeach</div>
  @endif
    ...
    <div class="comics-grid" id="library-grid">
      @foreach($libraries as $item)
        ...
        <article class="comic-card-sm" id="library-item-{{ $comic->id }}" style="position:relative;transition:.25s">
          ...
          <button type="button" class="library-remove" data-comic="{{ $comic->id }}" data-title="{{ $comic->title }}" style="position:absolute;top:8px;right:8px;z-index:4;width:32px;height:32px;border-radius:50%;border:1px solid rgba(239,68,68,.4);background:rgba(11,14,20,.82);color:#f87171;cursor:pointer">✕</button>
          <div style="padding:0 12px 12px">@if($item->lastReadChapter)<a href="{{ route('chapters.show',[$comic->slug,$item->lastReadChapter->slug]) }}" class="btn-spotlight-read" style="display:block;text-align:center;text-decoration:none;padding:8px 10px;font-size:12px">📖 Đọc tiếp Ch.{{ $item->lastReadChapter->chapter_number }}</a>@else<a href="{{ route('comics.show',$comic->slug) }}" class="btn-spotlight-sub" style="display:block;text-align:center;text-decoration:none;padding:8px 10px;font-size:12px">Xem chi tiết</a>@endif</div>
        </article>
        @endif
      @endforeach
    </div>
